...
window.addEventListener("DOMContentLoaded", function () {  
    ...
    var img_wrapper1 = document.querySelector(".img_wrapper1");
    ...
    //利用for循环生成circle
    for (var i = 0; i < img_wrapper1.children.length; i++) {
      ...
      li.addEventListener("click", function () {
        ...
        animate(  //实现点击小圆圈也可以切换图片
          img_wrapper1,
          -img_wrapper1.parentElement.offsetWidth * this.getAttribute("index")
        );
      });
    }
  ...
  //由于播放到最后一张图片后再点击下一张图片动画会返回第一张，并且画面非常不好看，要实现无缝切换图片即需将第一张图片复制放到最后面。由于添加小圆圈的代码是在复制第一张图片之前执行的，因此小圆圈的数量不会受影响。
    ...
    var firstImg = img_wrapper1.children[0].cloneNode(true);//创建节点
    img_wrapper1.appendChild(firstImg);//添加节点
  ...
  //当我们鼠标不在图片的盒子中时，不显示切换图片的两个按钮，这里实现鼠标在盒子上方时按钮显示，并且暂停自动播放图片的功能
    ...
    wrapper1.addEventListener("mouseleave", function () {
    ...
    next.addEventListener("click", function () {
      if (flag) {  //flag判断是否上一个动画已完成
        ...
        if (num == img_wrapper1.children.length - 1) {
          img_wrapper1.style.left = 0 + "px";
          num = 0;
        }
        num++;
        ...
        animate(
          img_wrapper1,
          -num * img_wrapper1.parentElement.offsetWidth,
          function () {
            flag = true;
          }
  ...
  //上一张图片的按钮代码与下一张的相似
    prev.addEventListener("click", function () {
      if (flag) {
        if (num == 0) {
          num = img_1.children.length - 1;
          img_wrapper1.style.left =
            -num * img_wrapper1.parentElement.offsetWidth + "px";
        }
        num--;
        animate(
          img_wrapper1,
